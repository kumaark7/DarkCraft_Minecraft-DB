...
import { hostPaths, nearestHostSample, type HostSample } from '@/utils/hostHistory';
...
export function HostHistoryGraph({ samples }: { samples: HostSample[] }) {
  const id = useId();
    ...
    <section aria-label="Live host CPU and RAM monitor" className="mt-4 border-t border-border/50 pt-3 min-w-0">
      <div className="flex flex-wrap items-center justify-between gap-2 text-[10px] text-muted-foreground mb-2">
        <div className="flex items-center gap-3">
          ...
          <span>{frozen ? 'Paused' : !latest ? 'Collecting…' : available ? 'Live · ~5s' : 'Unavailable · retrying'}</span>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1 min-w-0 rounded-sm focus-visible:outline focus-visible:outline-1 focus-visible:outline-offset-4 focus-visible:outline-ring"
          ...
          <svg role="img" aria-label="Host CPU and RAM usage over the last five minutes" viewBox="0 0 100 100" preserveAspectRatio="none"
            className="w-full h-32 overflow-visible" onPointerMove={event => {
              const bounds = event.currentTarget.getBoundingClientRect();
              if (bounds.width && history.length) setSelectedTime(history[nearestHostSample(history, (event.clientX - bounds.left) / bounds.width)].timestamp);
            }} onPointerDown={event => {
              const bounds = event.currentTarget.getBoundingClientRect();
              if (bounds.width && history.length) setSelectedTime(history[nearestHostSample(history, (event.clientX - bounds.left) / bounds.width)].timestamp);
            ...
            {selectedTime !== null && selected && latest && <line
              x1={100 - (latest.timestamp - selected.timestamp) / 3000} x2={100 - (latest.timestamp - selected.timestamp) / 3000}
              y1="0" y2="100" stroke="currentColor" className="text-muted-foreground/50" strokeDasharray="3 4" vectorEffect="non-scaling-stroke" />}
            {series.flatMap(item => hostPaths(history, item.metric).map((d, index) => <path key={`${item.label}-${index}`} d={d} fill="none" stroke={item.color}
              strokeWidth="2" strokeLinecap="round" strokeDasharray={item.dash} vectorEffect="non-scaling-stroke" />))}
          </svg>
          <div aria-hidden="true" className="flex justify-between mt-1 text-[9px] text-muted-foreground"><span>−5m</span><span>−2.5m</span><span>{latest ? time(latest.timestamp) : 'Now'}</span></div>
        </div>
      </div>
      ...
      <p className="text-[9px] text-muted-foreground mt-1">5-minute window · hover or tap to inspect</p>
    </section>
  );
}
