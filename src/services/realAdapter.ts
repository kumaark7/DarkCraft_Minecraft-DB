...
export function createRealServices(client: ApiClient) {
  ...
  const playerService: IPlayerService = {
    ...
    addWhitelistPlayer: (id, username, edition = 'java') => client.post(safeServerPath(id, '/players/whitelist'), { username, edition }),
    removeWhitelistPlayer: (id, username, edition = 'java', uuid) => client.delete(safeServerPath(id, '/players/whitelist'), { username, edition, uuid }),
  ...
  const globalService: IGlobalService = {
    getHostHistory: range => client.get<import('../types').HostMetricSample[]>('/host/metrics', { range }),
